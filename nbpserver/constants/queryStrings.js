...
const CREATE_CLIENT='CREATE(a:Client {firstName:$Ime,lastName:$Prez,username:$User,password:$Pass,type:"client",isActive:false,currentLat:$Lat,currentLng:$Lng,currentLocation:$Loc}) return a';
...
const FINISH_RIDE='match (d:Driver)-[r1]->(r:Ride)<-[r2]-(c:Client) where id(c)=$CID and id(d)=$DID and id(r)=$RID set d.isActive=false,c.isActive=false,d.currentLat=$DLat,d.currentLng=$DLng,d.currentLocation=$DLoc,c.currentLat=$DLat,c.currentLng=$DLng,c.currentLocation=$DLoc,r.endTime=$ETime,r.isCanceled=$Canc return r';
const CANCEL_RIDE='match (d:Driver)-[r1]->(r:Ride)<-[r2]-(c:Client) where id(c)=$CID and id(d)=$DID and id(r)=$RID set d.isActive=false,c.isActive=false,r.endTime=$ETime,r.isCanceled=$Canc return r';
...
const UPDATE_CLIENT_TRUE='match (c:Client) where id(c)=$CID set c.isActive=true,c.currentLat=$Lat,c.currentLng=$Lng,c.currentLocation=$Loc';
